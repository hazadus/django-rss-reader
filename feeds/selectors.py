...
def get_entry_queryset(mode: str) -> QuerySet:
    """
    ...
    return MODE_QUERYSETS[mode]


def get_entry_count(mode: str) -> int:
    """
...
def get_previous_entry(entry: Entry, queryset: QuerySet = Entry.objects.all()):
    ...
    queryset = queryset.filter(pub_date__gt=entry.pub_date)
    if queryset.exists():
...
def get_next_entry(entry: Entry, queryset: QuerySet = Entry.objects.all()):
    ...
    queryset = queryset.filter(pub_date__lt=entry.pub_date)
    if queryset.exists():
        return queryset.first()
    return None


def get_all_feeds() -> QuerySet:
    return Feed.objects.all().prefetch_related("entries")
